...
class BaseSPARQL:
    ...
    def text_query(self, variable, triples, lang='en', show_query=False):
        ...
        data = self.execution(self._text_tpl % (
            self.q_mark(variable), triples), show_query)
        result = []
        for d in data:
            res = {}
    ...
    def count_query(self, variable, triples):
        ...
        query = 'count(%s) as %s' % (
            self.q_mark(variable), self.q_mark(variable))
...
class NameSPARQL(BaseSPARQL):
    ...
    def wiki2dbpedia(self, wiki_uri):
        if wiki_uri.__contains__('https://en.wikipedia.org/wiki/'):
            ...
            q = self.triple('?s', self.uri(
                'http://xmlns.com/foaf/0.1/isPrimaryTopicOf'), self.uri(wiki_uri))
    ...
    def resource_filter(self, data):
        result = [d for d in data if d.__contains__(
            'http://dbpedia.org/resource/')]
        # remove categories
        result = [d for d in result if not d.__contains__(
    ...
    def name2entities(self, name):
        ...
        t2 = self.triple('?s', self.uri(RDFS.label),
                         self.literal(name.title()))
