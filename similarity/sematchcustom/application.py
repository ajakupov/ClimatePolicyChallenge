...
class Matcher:
    ...
    def type_links(self, word, lang='eng'):
        ...
        if self._expansion:
            synsets = list(set(itertools.chain.from_iterable(
                [self._yago.synset_expand(s) for s in synsets])))
        links = []
        for s in synsets:
            link_dic = {}
    ...
    def query_process(self, query):
        ...
        entity_filter = list(itertools.chain.from_iterable(
            [e.lower().split() for e in entities]))
        ...
        concept_uris = [list(itertools.chain.from_iterable(
            [s['lod'] for s in self.type_links(c)])) for c in concepts]
        concept_uris = list(itertools.chain.from_iterable(concept_uris))
        entity_uris = list(itertools.chain.from_iterable(
            map(self._linker.name2entities, entities)))
    ...
    def match_concepts(self, concepts, lang='en'):
        ...
        for i in xrange(0, len(concepts), 5):
            results.extend(self._query_graph.type_query(
                concepts[i:i + 5], lang, self._show_query))
        result_dic = {}
        for res in results:
    ...
    def match_type(self, query, lang='eng'):
        ...
        concept_uris = []
        for w in words:
            concepts = list(itertools.chain.from_iterable(
                [s['lod'] for s in self.type_links(w, lang)]))
    ...
    def match_entity_type(self, query):
        ...
        for e in entities:
            for i in xrange(0, len(concepts), 5):
                results.extend(self._query_graph.type_entity_query(
                    concepts[i:i + 5], e, self._show_query))
        result_dic = {}
        for res in results:
            if res['uri'] not in result_dic:
                result_dic[res['uri']] = res
...
class SimClassifier:
    ...
    def pick_sim_model(self, sim_model):
        def weighted(x, y): return self.weighted_similarity(x, y)
        def max_sim(x, y): return self.max_similarity(x, y)
        def average(x, y): return self.average_similarity(x, y)
    ...
    def train(cls, corpus, sim_metric, feature_num=5, sim_model='weighted'):
        ...
        for sent, cat in corpus:
            cat_word.setdefault(cat, []).extend(
                word_process(word_tokenize(sent)))
    ...
    def average_similarity(self, word, category):
        ...
        """
        sum_score = sum(map(lambda x: self._sim_metric(
            word, x), self._cat_features[category]) + [0.0])
    ...
    def classify_single(self, sent, feature_model='max'):
        ...
        for c in self._categories:
            if feature_model == 'max':
                score[c] = max([self.category_similarity(w, c)
                                for w in feature_words] + [0.0])
            else:
                score[c] = sum([self.category_similarity(w, c)
                                for w in feature_words] + [0.0])
...
class TextPreprocessor(BaseEstimator, TransformerMixin):
    ...
    def similarity(self, tokens, feature):
        def sim(x): return self._word_sim(feature, x)
...
class SimSVMClassifier:
    ...
    def train(cls, X, y, word_sim_metric, classifier=LinearSVC,
        ...
        def build():
            ...
            model = Pipeline([
                ('preprocessor', TextPreprocessor(
                    corpus, word_sim_metric, feature_num, feature_type)),
        ...
        model, secs = build()
        if verbose:
            print("Complete model building in {:0.3f} seconds".format(secs))
